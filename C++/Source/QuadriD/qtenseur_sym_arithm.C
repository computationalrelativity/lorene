/*
 *  Arithmetics functions for the Qtenseur_sym class.
 *
 *  These functions are not member functions of the Qtenseur_sym class.
 *
...
char qtenseur_sym_arithm_C[] = "$Header$" ;
...
Qtenseur_sym operator+(const Qtenseur_sym & t) {
...
Qtenseur_sym operator-(const Qtenseur_sym & t) {
   ...
   assert (t.get_etat() != ETATNONDEF) ;
	...
	Qtenseur_sym res(*(t.get_mp()), t.get_valence(), t.get_type_indice(), 
			*(t.get_triad()) ) ; 
...
Qtenseur_sym operator+(const Qtenseur_sym & t1, const Qtenseur_sym & t2) {
	...
	return t2 ;
	    ...
	    Qtenseur_sym res(*(t1.get_mp()), t1.get_valence(), 
...
Qtenseur_sym operator*(double x, const Qtenseur_sym& t) {
...
Qtenseur_sym operator* (const Qtenseur_sym& t, double x) {
    return x * t ;
}

Qtenseur_sym operator*(int m, const Qtenseur_sym& t) {
    return double(m) * t ; 
}


Qtenseur_sym operator* (const Qtenseur_sym& t, int m) {
    return double(m) * t ;
}
...
Qtenseur_sym operator/ (const Qtenseur_sym& t1, const Qtenseur& t2) {
	...
	cout << "Division by 0 in Qtenseur_sym / Qtenseur !" << endl ;
	abort() ; 
    }
    if (t1.get_etat() == ETATZERO) {
    	return t1 ;
    ...
    Qtenseur_sym res(*(t1.get_mp()), t1.get_valence(), t1.get_type_indice(), 
		    *(t1.get_triad()) ) ; 
...
Qtenseur_sym operator/ (const Qtenseur_sym& t, double x) {
    ...
    if ( x == double(0) ) {
	cout << "Division by 0 in Qtenseur_sym / double !" << endl ;
	abort() ;
    }
