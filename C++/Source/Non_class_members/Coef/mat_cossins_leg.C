 ...
 * ---------------------------
 *  double* mat_cossins_leg : pointeur sur le tableau contenant l'ensemble
 *			        (pour les np/2+1 valeurs de m) des 
 *				matrices de passage.
 *				La dimension du tableau est (np/2+1)*nt^2
 *				Le stokage est le suivant: 
 *
 *		mat_cossins_leg[ nt*nt* m + nt*l + j] = A_{mlj}
 *			    
...
// Variable de loch
int loch_mat_cossins_leg = 0 ;

//******************************************************************************

double* mat_cossins_leg(int np, int nt) {
...
//    #pragma critical (loch_mat_cossins_leg)
    ...
    if (indice == -1) {		   
	if ( nb_dejafait >= NMAX ) {
	    cout << "mat_cossins_leg: nb_dejafait >= NMAX : "
