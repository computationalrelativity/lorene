...
Vector Isol_hor::radial_vect_hor_ih()  {
...
Vector Isol_hor::beta_bound_cart_ih() {


  const Map& map = ff.get_mp() ; 

  Vector tmp_vect = nn() * radial_vect_hor_ih() ;
...
double Isol_hor::radius_hor_ih()  {
  ...
  double resu =  map_affine.integrale_surface(darea_hor_ih(), 1.0000000001) / (4. * M_PI);
...
double Isol_hor::ang_mom_hor_ih() {
  ...
  Scalar k_rphi = contract(contract( radial_vect_hor_ih(), 0, k_dd(), 0), 0, phi, 0) / (8. * M_PI) ;

  Scalar integrand = k_rphi * darea_hor_ih() ;   // we correct with the curved element of area 
...
double Isol_hor::mass_hor_ih() {
  
  double rr = radius_hor_ih() ;

  double  tmp = sqrt( pow( rr, 4) + 4 * pow( ang_mom_hor_ih(), 2) ) / ( 2 * rr ) ;
...
// Surface gravity
double Isol_hor::kappa_hor_ih() {
  
  double rr = radius_hor_ih() ;

  double jj = ang_mom_hor_ih() ;
