...
char mat_leg_cossincs_C[] = "$Header$" ;
 ...
 * ---------------------------
 *  double* mat_leg_cossin : pointeur sur le tableau contenant l'ensemble
...
// Variable de loch
int loch_mat_leg_cossin = 0 ;

//******************************************************************************

double* mat_leg_cossins(int np, int nt) {
...
//    #pragma critical (loch_mat_leg_cossins)
    {
...
// Si le calcul n'a pas deja ete fait, il faut le faire : 
    if (indice == -1) {		   
	if ( nb_dejafait >= NMAX ) {
	    cout << "mat_legp_cossins: nb_dejafait >= NMAX : "
