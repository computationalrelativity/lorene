...
#include "unites.h"
    ...
    if (vv.get_valence() != 1) {
	cout << 
    "des_coupe_vect_x: the Tenseur must be of valence 1 (vector) !" << endl ;
	abort() ; 
    }
	...
	assert(defsurf->get_mp() == vv.get_mp()) ; 
...
#include "unites.h"
    ...
    if (vv.get_valence() != 1) {
	cout << 
    "des_coupe_vect_y: the Tenseur must be of valence 1 (vector) !" << endl ;
	abort() ; 
    }
