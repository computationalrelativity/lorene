...
/*
 * Transformation en sin(2*l*theta) inverse sur le deuxieme indice (theta)
 ...
 * LORENE
 *
 * Revision 2.0  1999/02/22  15:41:05  hyc
 * *** empty log message ***
 *
 *
 * $Header$
...
void citsinp(const int* deg, const int* dimc, double* cf, const int* dimf,
		   double* ff)
{
...
// Tests de dimension:
    if (nt > n2f) {
	cout << "citsinp: nt > n2f : nt = " << nt << " ,  n2f = " 
	<< n2f << endl ;
	abort () ;
	exit(-1) ;
    }
    if (nt > n2c) {
	cout << "citsinp: nt > n2c : nt = " << nt << " ,  n2c = " 
	<< n2c << endl ;
	abort () ;
	exit(-1) ;
    }
    if ( (n1f > 1) && (n1c > n1f) ) {
	cout << "citsinp: n1c > n1f : n1c = " << n1c << " ,  n1f = " 
	<< n1f << endl ;
	abort () ;
	exit(-1) ;
    }
    if (n3c > n3f) {
	cout << "citsinp: n3c > n3f : n3c = " << n3c << " ,  n3f = " 
	<< n3f << endl ;
	abort () ;
	exit(-1) ;
    }
...
// boucle sur phi et r (les boucles vont resp. de 0 a max(dimc[0]-2,0) 
//			et 0 a dimc[2]-1)
