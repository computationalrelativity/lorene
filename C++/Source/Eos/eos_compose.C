...
// ----------------------------
Eos_CompOSE::Eos_CompOSE(FILE* fich) : Eos_tabul("CompOSE Eos")
{
  using namespace Unites ;
  char tmp_string_name[100] ;
  fread(tmp_string_name, sizeof(char), 100, fich) ;
  char tmp_string[160] ;
  fread(tmp_string, sizeof(char), 160, fich) ;
  tablename = tmp_string;

  cout << "++++++ New Eos_CompOSE constructor!!!" << endl ;

  // Files containing data and a test
  //---------------------------------
  string files = tmp_string ;
  string file_nb = files + ".nb" ;
  string file_thermo = files + ".thermo" ;

  ifstream in_nb(file_nb.data()) ;
  if (!in_nb) {
    cerr << "Eos_CompOSE::Eos_CompOSE(*FILE): " << endl ;
  ...
  for (int i=0; i<nbp; i++) {
    ...
    if ( (nb_fm3<0) || (rho_cgs<0) || (p_cgs < 0) ){
      cout << "Eos_CompOSE::Eos_CompOSE(*FILE): " << endl ;
