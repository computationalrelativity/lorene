 ...
 *  on another mapping.
 * Case where both Cmp's are antisymmetric with respect to their y=0 plane.
 */
...
char cmp_import_asymy_C[] = "$Header$" ;
 ...
 * LORENE
 *
 * Revision 2.0  2000/03/06  10:56:07  eric
 * *** empty log message ***
 *
 *
 * $Header$
...
void Cmp::import_asymy(int nzet, const Cmp& cm_d) {
	...
	*this = cm_d ; 
	...
	default : {  
	    cout << "Cmp::import_asymy : unexpected value of align_rel : " 
		 << align_rel << endl ; 
	    abort() ; 
	    break ; 
	}
...
void Cmp::import_anti_asymy(int nzet, const Cmp& cm_d) {
    ...
    if (cm_d.dzpuis != 0) {
	cout << 
	"Cmp::import_anti_asymy : the dzpuis of the Cmp to be imported"
	  << " must be zero !" << endl ; 
	abort() ; 
    }
...
void Cmp::import_align_asymy(int nzet, const Cmp& cm_d) {
    ...
    if (cm_d.dzpuis != 0) {
	cout << 
	"Cmp::import_align_asymy : the dzpuis of the Cmp to be imported"
	<< " must be zero !" << endl ; 
	abort() ; 
    }
