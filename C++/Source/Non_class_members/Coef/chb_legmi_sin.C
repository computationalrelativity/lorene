...
char chb_legmi_sin_C[] = "$Header$" ;

/*
 *  Calcule les coefficients du developpement (suivant theta) 
 *  en sin(j theta) 
 *  a partir des coefficients du developpement en fonctions
 *  associees de Legendre P_l^m(cos(theta)) (m impair)
 *  pour une une fonction 3-D antisymetrique par le retournement 
 *  (x, y, z) --> (-x, -y, z). 
 * 
 * Entree:
 * -------
 ...
 *
 *		    f(theta) = 
 *			    som_{l=m}^{nt-2} a_j P_j^m( cos(theta) )
 *			  
 *		(m impair) 
 *
 ...
 *
 *			f(theta) = som_{j=0}^{nt-2} c_j sin( j theta ) 
 *			  
