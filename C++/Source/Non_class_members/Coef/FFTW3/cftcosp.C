...
char cftcosp_C[] = "$Header$" ;


/*
 * Transformation en cos(2*l*theta) sur le deuxieme indice (theta)
 ...
 * LORENE
 *
 * Revision 2.0  1999/02/22  15:47:50  hyc
 * *** empty log message ***
 *
 *
 * $Header$
...
// Tests de dimension:
    if (nt > n2f) {
	cout << "cftcosp: nt > n2f : nt = " << nt << " ,  n2f = " 
	<< n2f << endl ;
	abort () ;
	exit(-1) ;
    }
    if (nt > n2c) {
	cout << "cftcosp: nt > n2c : nt = " << nt << " ,  n2c = " 
	<< n2c << endl ;
	abort () ;
	exit(-1) ;
    }
    if (n1f > n1c) {
	cout << "cftcosp: n1f > n1c : n1f = " << n1f << " ,  n1c = " 
	<< n1c << endl ;
	abort () ;
	exit(-1) ;
    }
    if (n3f > n3c) {
	cout << "cftcosp: n3f > n3c : n3f = " << n3f << " ,  n3c = " 
	<< n3c << endl ;
	abort () ;
	exit(-1) ;
    }
