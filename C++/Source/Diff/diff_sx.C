...
const Matrice& Diff_sx::get_matrice() const {
	...
	if (indice == max_points) {
	    cerr << "Diff_sx::get_matrice() : no space left!!" << '\n'
...
ostream& Diff_sx::operator>>(ostream& ost) const {

    ost << " Division by x " << endl ;
