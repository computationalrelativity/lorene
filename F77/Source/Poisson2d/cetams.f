...
C
C
	SUBROUTINE CETAMS(N,N64,INDEX,F,CS,CC)

	IMPLICIT double PRECISION (A-H,O-Z)

C	    
C	ROUTINE POUR LE CALCUL DES COEFFICIENTS DE TCHEBYTCHEV
C	DE N64 FONCTIONS  AVEC ECHANTILLONAGE
...
C		  DOIT ETRE=0, SI LA FONCTION EST IMPAIRE
C		  INDEX=1.
C	   F	 =TABLEAU CONTENANT LES N64 FONCTIONS QU'ON VEUT
C		  TRANSFORMER. LE TABLEAU DOIT CONTENIR LES ECHANTILLONS
C		  DES FONCTIONS EN 'PARALLEL'. (Voir la routine tfms).
C	   CS	 =TABLEAU DE TRAVAIL.
C	   CC	 =OUTPUT. COEFFICIENTS DE TCHEBYTCHEV DE N64 FONCTIONS F
C		  EN 'PARALLEL'.
C
C	DIMENSIONS MINIMALES DES TABLEAUX = (N64+1)*(N+4)
C......................................................................
C
C	Routine modifiee le 28/10/94
C
C
C $Id$
C $Log$
...
C Initial revision
C
c Revision 1.1  1997/10/23  08:21:42  eric
c Initial revision
c
C
C $Header$
...
C
	...
	PRINT 200,N
 200	FORMAT(10X,'DIMENSION DU TABLEAU SEN INSUFFISANTE DANS LA'
     , ,' ROUTINE CHEIM: N=',I4)
	CALL EXIT
  8	CONTINUE
	NC257=257
	IF(N64.GT.257) THEN
	PRINT*,'DIMENSIONS INSUFFISANTES DANS LA ROUT,CETAMS,N64=',N64
	CALL EXIT
	ENDIF
C
	X0=0
...
C
C
C	    TRANSFORMATION DE TCHEBYTCHEV DE LA FONCTION PAIRE.
C
	DO 3 M=1,N64
