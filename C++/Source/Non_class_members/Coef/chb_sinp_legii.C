 ...
 *
 *			f(theta) = som_{j=0}^{nt-2} c_j sin( 2j theta ) 
 *
 ...
 *		    phi et r respectivement. On a c_0 = c_{nt-1} = 0. 
 *
 * Sortie:
 * -------
 ...
 *		    NB: pour j<(m+1)/2 ou l=nt-1,  a_j = 0
 *
 * NB:
 * ---
...
void chb_sinp_legii(const int* deg , const double* cfi, double* cfo) {
...
int k2, l, jmin, j, i, m ;
    ...
    double* aa = mat_sinp_legii(np, nt) ;
	...
	jmin =  1  ;  // pour m=1, aa_lj = 0 pour j<l
