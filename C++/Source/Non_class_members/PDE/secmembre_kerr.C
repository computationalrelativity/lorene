...
Sym_tensor secmembre_kerr ( const Sym_tensor& hij, const Sym_tensor& aa,const Scalar& nn,const Scalar& ppsi,const Vector& bb) { 
  ...
  Scalar qpig(hij.get_mp()); qpig.annule_hard(); qpig.std_spectral_base();
  ...
  tmp = psi4 * strain_tens.trace(tgam) ; // S = S_i^i  // Ces lignes sont probablement liees a l'ajout de matière: A verifier!!!
  ...
  source_hh -= nn / (psi4*ppsi*ppsi) * sym_tmp ; 
