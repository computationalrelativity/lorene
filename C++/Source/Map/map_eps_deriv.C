/*
 * Computations of Scalar partial derivatives for a Map_eps mapping
 */
...
void Map_eps::dsdr(const Scalar& uu, Scalar& resu) const {
  ...
  if (uu.get_etat() == ETATZERO) {
    ...
    resu.set_spectral_base( uuva.dsdx().get_base() ) ; // same basis as d/dxi
			...
			//       1/sin(theta) d/dphi         //
			//-----------------------------------//
void Map_eps::stdsdp(const Scalar& ci, Scalar& resu) const {
...
void Map_eps::srstdsdp(const Scalar& uu, Scalar& resu) const {
...
void Map_eps::dsdt(const Scalar& ci, Scalar& resu) const {
...
void Map_eps::srdsdt(const Scalar& uu, Scalar& resu) const {
  assert (uu.get_etat() != ETATNONDEF) ; 
  assert (uu.get_mp().get_mg() == mg) ; 
