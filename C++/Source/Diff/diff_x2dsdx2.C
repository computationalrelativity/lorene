/*
 *  Methods for the class Diff_x2dsdx2
 *
...
Diff_x2dsdx2::Diff_x2dsdx2(int base_r, int nr) : Diff(base_r, nr) {
    initialize() ;
}

Diff_x2dsdx2::Diff_x2dsdx2(const Diff_x2dsdx2& diff_in) : Diff(diff_in) {
    assert (nap != 0) ;
} 

Diff_x2dsdx2::~Diff_x2dsdx2() {}

void Diff_x2dsdx2::initialize() {
    if (nap == 0) {
...
void Diff_x2dsdx2::operator=(const Diff_x2dsdx2& diff_in) {
    assert (nap != 0) ;
    Diff::operator=(diff_in) ;

}

const Matrice& Diff_x2dsdx2::get_matrice() const {
	...
	if (indice == max_points) {
	    cerr << "Diff_x2dsdx2::get_matrice() : no space left!!" << '\n'
...
ostream& Diff_x2dsdx2::operator>>(ostream& ost) const {
