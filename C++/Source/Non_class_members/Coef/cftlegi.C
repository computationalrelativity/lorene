...
char cftlegi_C[] = "$Header$" ;

/*
 * Transformation en fonctions de Legendre associees impaires sur le deuxieme 
 *  indice (theta) d'un tableau 3-D representant une fonction antisymetrique 
 ...
 * Initial revision
 *
 * Revision 2.0  1999/02/22  15:46:55  hyc
 * *** empty log message ***
 *
 *
 * $Header$
...
void cftlegi(const int* deg, const int* dimf, double* ff, const int* dimc,
		double* cf)
{
...
//--------------------------------------------------------------
    ...
    cftcossinci(deg, dimf, ff, dimc, cf_cs) ;

//--------------------------------------------------------------
// 2/ Transformation  cos((2j+1) theta)/sin(2j theta) ---> Legendre 
//--------------------------------------------------------------
