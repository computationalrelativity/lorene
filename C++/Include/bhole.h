 ...
 * must have,  at least two shells. The horizon (i.e. the surface where $N=0$) 
...
class Bhole {
	...
	Tenseur n_auto ; /// Part of $N$ generated by the hole.
	Tenseur n_comp ; ///Part of $N$ generated by the companion hole.
	Tenseur n_tot ; /// Total $N$.
	...
	Tenseur grad_n_tot ; /// Total gradient of $N$.
	...
	/**
	 * Imports the part of $N$ due to the companion hole {\tt comp}. The 
	 * total $N$ is then calculated.
	 * 
	 ...
	 * masses,  calculated by the asymptotic behaviours of respectively $\Psi$ and $N$.
	 * 
	 ...
	 * f\left(\theta, \phi\right)$ on the horizon, where $f$ is the value of $\Psi$ on 
	 ...
	 * where $\vec{m} = \frac{\partial}{\partial \phi}$ and $H$ denotes the horizon.
	 * 
	 *{\bf WARNING} It supposes that the boost is zero and should only be 
	 * used for an isolated black hole..
	 */
...
class Bhole_binaire {
	 ...
	 * the fields generated by each holes ($a = 1, 2$). The boundary conditions are 
	 * such that $N=0$ on both horizons.
	 ...
	 * \end{equation}
	 * The fields are the total values excpet those with subscript $_a$, which are 
	 * the fields generated by each holes ($a = 1, 2$). The boundary conditions are such that 
	 * $\partial_r \Psi = -\frac{1}{2 r_0} f\left(\theta, \phi\right)$ on both horizons,  $r_0$ being 
	 * the radii of those horizons and $f$ the value of $\Psi$ on the 
	 ...
	 * The fields are the total values excpet those with subscript $_a$, which are 
	 * the fields generated by each holes ($a = 1, 2$). The companion contributions are then 
	 * obtained.
	 ...
	 * close to the horizons (division by $N=0$) for it is done in the coefficient space 
    ...
    public:
	  ...
	  * @param precis [input] : precision for the convergence (on $N$).
	  * @param relax [input] : relaxation parameter.
	  */
