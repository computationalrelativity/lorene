...
int main(){
  ...
  string resdir = "Results";	/* default value */
  char *resdir_tmp = NULL;
  if ( read_variable (NULL, "resdir", &resdir_tmp) == 0)
    resdir = resdir_tmp ;
...
//----------------------------------------------------------------------
void compare_analytic (Et_rot_bifluid& star, int adapt, string resdir)
{
  using namespace Unites ;
  using Lorene::beta ;
  ...
  DIR *thisdir;
  if ( (thisdir = opendir(resdir.c_str())) == NULL)
    if (mkdir (resdir.c_str(), S_IREAD|S_IWRITE|S_IEXEC) == -1)
      {
