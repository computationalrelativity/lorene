 ...
 * P_l^m(cos(theta)) paires (i.e. telles que l-m est pair) avec m pair.
 * 
 ...
 * ---------------------------
 *  double* mat_cosp_legpp : pointeur sur le tableau contenant l'ensemble
 *			        (pour les np/2+1 valeurs de m: m=0,2,...,np) des 
 *				matrices de passage.
 *				La dimension du tableau est (np/2+1)*nt^2
 *				Le stokage est le suivant: 
 *
 *		mat_cosp_legpp[ nt*nt* m/2 + nt*l + j] = A_{mlj}
 *			    
 *				ou A_{mlj} est defini par
 *
 *   cos(2*j*theta) = som_{l=m/2}^{nt-1} A_{mlj} P_{2l}^m( cos(theta) )
 *
...
// Variable de loch
int loch_mat_cosp_legpp = 0 ;

//******************************************************************************

double* mat_cosp_legpp(int np, int nt) {
...
int i, indice,  j,  j2,  m,  l ;
    ...
    #pragma critical (loch_mat_cosp_legpp)
    {
...
// Calcul des cos(2*j*theta)  aux points de collocation
...
// Recherche des fonctions de Legendre associees d'ordre m :
		...
		for (j=0; j<nt; j++) {	// boucle sur les cos(2j theta)
			
//... produit scalaire de cos(2j theta) par P_{2l}^m(cos(theta)) 
...
//	    l'integrale (routine int1d_chebp) : 		
		...
		}	// fin de la boucle sur j  (indice de cos(2j theta) )
