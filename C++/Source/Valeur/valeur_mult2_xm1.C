/*
 * Computation of the multiplication by (x-1)^2 in the external compactified
 * domain
 *
 * for:
 *   - Valeur
...
char valeur_mult2_xm1_C[] = "$Header$" ;

/*
 * $Id$
 * $Log$
 * Revision 1.3  2014/10/13 08:53:50  j_novak
...
static void (*_mult2_xm1[MAX_BASE])(Tbl *, int&) ;
static int nap = 0 ;

    if (nap==0) {
	nap = 1 ;
	for (int i=0 ; i<MAX_BASE ; i++) {
	   _mult2_xm1[i] = _mult2_xm1_identite ;
	}
	
	// Les routines existantes cas UNSURR
	_mult2_xm1[R_CHEBU >> TRA_R] = _mult2_xm1_cheb ;
    }
	...
	_mult2_xm1[base_r](t[l], base.b[l]) ;
    }
