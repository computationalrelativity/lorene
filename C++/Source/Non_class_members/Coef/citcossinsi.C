...
char citcossinsi_C[] = "$Header$" ;

/*
 * Transformation inverse sin((2*l+1)*theta) ou cos(2*l*theta)(suivant la 
 ...
 * LORENE
 *
 * Revision 2.0  1999/02/22  15:42:15  hyc
 * *** empty log message ***
 *
 *
 * $Header$
...
void citcossinsi(const int* deg, const int* dimc, double* cf, const int* dimf,
		   double* ff)
{
...
// Tests de dimension:
    if (nt > n2f) {
	cout << "citcossinsi: nt > n2f : nt = " << nt << " ,  n2f = " 
	<< n2f << endl ;
	abort () ;
	exit(-1) ;
    }
    if (nt > n2c) {
	cout << "citcossinsi: nt > n2c : nt = " << nt << " ,  n2c = " 
	<< n2c << endl ;
	abort () ;
	exit(-1) ;
    }
    if (n1c > n1f) {
	cout << "citcossinsi: n1c > n1f : n1c = " << n1c << " ,  n1f = " 
	<< n1f << endl ;
	abort () ;
	exit(-1) ;
    }
    if (n3c > n3f) {
	cout << "citcossinsi: n3c > n3f : n3c = " << n3c << " ,  n3f = " 
	<< n3f << endl ;
	abort () ;
	exit(-1) ;
    }
...
//-----------------------------------------------------------------------
//  partie cos(m phi) avec m pair : transformation sin((2 l + 1) theta) inverse
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
//  partie sin(m phi) avec m pair : transformation sin( (2 l + 1) theta) inverse
//-----------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//  partie cos(m phi) avec m impair : transformation cos(2 l theta) inv.
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//  partie sin(m phi) avec m impair : transformation cos(2 l theta) inv.
//--------------------------------------------------------------------------
