...
char mat_cossincp_legp_C[] = "$Header$" ;
 ...
 * ---------------------------
 *  double* mat_cossincp_legp : pointeur sur le tableau contenant l'ensemble
 *			        (pour les np/2+1 valeurs de m) des 
 *				matrices de passage.
 *				La dimension du tableau est (np/2+1)*nt^2
 *				Le stokage est le suivant: 
 *
 *		mat_cossincp_legp[ nt*nt* m + nt*l + j] = A_{mlj}
 *			    
 ...
 * LORENE
 *
 * Revision 2.0  1999/02/22  15:34:45  hyc
 * *** empty log message ***
 *
 *
 * $Header$
...
// Variable de loch
int loch_mat_cossincp_legp = 0 ;

//******************************************************************************

double* mat_cossincp_legp(int np, int nt) {
...
//    #pragma critical (loch_mat_cossincp_legp)
    { 
    ...
    if (indice == -1) {		   
	if ( nb_dejafait >= NMAX ) {
	    cout << "mat_cossincp_legp: nb_dejafait >= NMAX : "
