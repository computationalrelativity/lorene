...
char cftlegip_C[] = "$Header$" ;

/*
 * Transformation en fonctions de Legendre associees sur le deuxieme indice
 *  (theta) d'un tableau 3-D representant une fonction 3-D antisymetrique
 *  par rapport au plan equatorial z = 0 et symetrique par le retournement 
 *  (x, y, z) --> (-x, -y, z). 
 *
 *
 * Entree:
 ...
 * LORENE
 *
 * Revision 2.0  2000/10/04  14:40:56  eric
 * *** empty log message ***
 *
 *
 * $Header$
...
void cftlegip(const int* deg, const int* dimf, double* ff, const int* dimc,
		double* cf)
{
...
//--------------------------------------------------------------
// 1/ Transformation esp. des configurations --> cos((2j+1) theta) 
//--------------------------------------------------------------

    cftcosi(deg, dimf, ff, dimc, cf_cs) ;

//--------------------------------------------------------------
// 2/ Transformation  cos((2j+1) theta)  ---> Legendre 
//--------------------------------------------------------------

    chb_cosi_legip(deg , cf_cs, cf) ;
