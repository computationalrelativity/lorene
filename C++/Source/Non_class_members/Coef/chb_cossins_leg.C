 ...
 *  developpement en cos(j*theta) [m impair] / sin(j * theta) [m pair]
 *  representant une fonction 3-D.
 * 
 * Entree:
 * -------
 ...
 *
 *	pour m impair:	f(theta) = som_{j=0}^{nt-1} c_j cos( j theta ) 
 *			  
 *	pour m pair:	f(theta) = som_{j=0}^{nt-2} c_j sin( j theta ) 
 *
...
void chb_cossins_leg(const int* deg , const double* cfi, double* cfo) {
...
// Recherche de la matrice de passage  cos/sin --> Legendre 
    double* aa = mat_cossins_leg(np, nt) ;
...
//	}
//	arrete() ; 
//	aat += maa ;	// pointeur sur la nouvelle matrice de passage	
//    }			
//##    
