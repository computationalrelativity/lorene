...
//Cas dzpuis == 2
Matrice _cl_helmholtz_minus_pseudo_1d_r_chebu_deux (const Matrice &source) {
...
void Ope_helmholtz_minus_pseudo_1d::do_ope_cl() const {
  if (ope_mat == 0x0)
    do_ope_mat() ;
  ...
  static Matrice (*cl_helmholtz_minus_pseudo_1d[MAX_BASE])(const Matrice&, int);
  static int nap = 0 ;
  ...
  ope_cl = new Matrice(cl_helmholtz_minus_pseudo_1d[base_r](*ope_mat, dzpuis)) ;
}
