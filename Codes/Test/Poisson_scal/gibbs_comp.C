/*
 * 
 * Test of scalar Poisson equation for a compact non-spherical source.
 * 
 */
 ...
 * Initial revision
 *
 * Revision 1.1  2000/02/15  14:51:27  phil
 * Initial revision
 *
 *
 * $Header$
...
void main() {
    ...
    for (int boucle = 0 ; boucle<borne ; boucle ++) {
	...
	val_so = pl_zin/pow(R, 2*l+3)
		 *((2*l+5)*(2*l+3)-(2*l+3)*(4*l+10)*pow(r, 2.)/pow(R, 2.)) ;
	val_so.annule(nz-1, nz-1) ;
	Cmp source(mapping) ;
	source =  val_so ;
	...
	val_sol = pl_zin*pow(r, 2.)/2/pow(R, 2*l+3)
		*(2*l+5-(2*l+3)*pow(r, 2.)/pow(R, 2.)) ;

	Valeur val_sol_zec(&grille) ;
	val_sol_zec = pl_zec/pow(r, l+1) ;
